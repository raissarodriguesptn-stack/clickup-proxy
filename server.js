...
http.createServer((req, res) => {
  ...
  const r = https.request({ hostname: 'api.clickup.com', path: fp, method: 'GET', headers: { 'Authorization': t } }, pr => {
    let b = '';
    pr.on('data', c => b += c);
    pr.on('end', () => { res.writeHead(pr.statusCode, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }); res.end(b); });
  });
  r.on('error', e => { res.writeHead(500); res.end(JSON.stringify({ error: e.message })); });
  r.end();
}).listen(process.env.PORT || 3000, '0.0.0.0', () => console.log('proxy ok porta', process.env.PORT || 3000));
